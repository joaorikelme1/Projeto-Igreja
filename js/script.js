...
function seed() {
  ...
  DB.set('iv_users', [{
    ...
    name:'João Administrador', firstLogin:false
  }]);
  DB.set('iv_trips',    []);
  DB.set('iv_payments', []);
  DB.set('iv_seats',    {});
...
function toast(msg, type='in', dur=3400) {
  ...
  el.innerHTML = `<span class="toast-ico">${icons[type]||'ℹ️'}</span><span class="toast-msg">${msg}</span><button class="toast-x" onclick="this.parentElement.remove()">✕</button>`;
  box.appendChild(el);
...
function buildAdminSidebar(active) {
  ...
  nav.innerHTML=`
    ...
    <a href="dashboard-admin.html" class="nav-a ${active==='dash'?'on':''}"><span class="ico">📊</span>Dashboard</a>
    <a href="viajantes.html"       class="nav-a ${active==='viaj'?'on':''}"><span class="ico">👥</span>Viajantes</a>
    <div class="sb-sec">Gestão</div>
    <a href="pagamento.html"    class="nav-a ${active==='pag'?'on':''}"><span class="ico">💳</span>Pagamentos</a>
    <a href="transporte.html"   class="nav-a ${active==='trans'?'on':''}"><span class="ico">🚌</span>Transporte</a>
    <a href="hotel.html"        class="nav-a ${active==='hotel'?'on':''}"><span class="ico">🏨</span>Hotel</a>
    <div class="sb-sec">Sistema</div>
    <a href="cadastros.html" class="nav-a ${active==='cadastros'?'on':''}"><span class="ico">🗂️</span>Cadastro Global</a>
    <a href="configuracoes.html" class="nav-a ${active==='cfg'?'on':''}"><span class="ico">⚙️</span>Configurações</a>`;
}
function buildTravelerSidebar(active) {
  ...
  nav.innerHTML=`
    ...
    <a href="dashboard-viajante.html" class="nav-a ${active==='dash'?'on':''}"><span class="ico">🏠</span>Início</a>
    <a href="pagamento.html"          class="nav-a ${active==='pag'?'on':''}"><span class="ico">💳</span>Pagamento</a>`;
}
  ...
  ov.innerHTML=`
    <div class="modal" style="max-width:480px">
      <div class="modal-head" style="background:linear-gradient(135deg,var(--navy),var(--navy-light));border-radius:var(--r-lg) var(--r-lg) 0 0">
        <div>
          <div class="modal-title" style="color:#fff">✈️ Suas Viagens</div>
      ...
      <div class="modal-foot"><button class="btn btn-outline" onclick="logout()">🚪 Sair</button></div>
    </div>`;
  ...
  const cards=trips.length
    ? trips.map(t=>`
      <div onclick="adminSelectTrip('${t.id}')" class="trip-card" data-id="${t.id}"
        ...
        <div style="flex:1">
          <div style="font-weight:700;font-size:.94rem;color:var(--text)">${t.name}</div>
          <div style="font-size:.76rem;color:var(--text-3);margin-top:.18rem">📍 ${t.destination||'—'} &nbsp;|&nbsp; 📅 ${fmtD(t.date)} &nbsp;|&nbsp; 👥 ${(t.travelers||[]).length}</div>
        </div>
        <div style="display:flex;gap:.35rem;flex-shrink:0">
          <button class="btn btn-outline btn-sm btn-icon" onclick="event.stopPropagation();openCreateTrip('${t.id}')">✏️</button>
          <button class="btn btn-danger btn-sm btn-icon" onclick="event.stopPropagation();deleteTrip('${t.id}')">🗑️</button>
        </div>
      </div>`).join('')
    : `<div class="alert al-sky"><span class="al-ico">ℹ️</span><span>Nenhuma viagem cadastrada. Crie a primeira!</span></div>`;
  ov.innerHTML=`
    <div class="modal" style="max-width:560px">
      <div class="modal-head" style="padding:1.25rem 1.5rem">
        <div>
          <div class="modal-title">✈️ Selecionar Viagem</div>
          <div style="font-size:.76rem;color:var(--text-3);margin-top:.2rem">Selecione ou crie uma viagem para gerenciar</div>
        </div>
        <button class="btn btn-red btn-sm" onclick="openCreateTrip()">➕ Nova Viagem</button>
      </div>
      <div class="modal-body" style="display:flex;flex-direction:column;gap:.6rem;max-height:55vh;overflow-y:auto">${cards}</div>
      <div class="modal-foot" style="justify-content:space-between">
        <button class="btn btn-outline btn-sm" onclick="openGlobalCadastros()">🗂️ Gerenciar Viajantes</button>
      ...
      <td style="padding:.85rem 1rem">
        <div style="display:flex;align-items:center;gap:.65rem">
          ...
          <div>
            ...
            <div style="font-size:.72rem;color:var(--text-3)">${u.married&&u.spouseName?'💍 '+u.spouseName:''} ${u.hasKids?'· 👶 '+(u.kids||[]).length+' filho(s)':''}</div>
          </div>
        </div>
      </td>
      <td style="padding:.85rem 1rem;font-size:.8rem;color:var(--text-3)">${CPF.format(u.cpf)}</td>
      <td style="padding:.85rem 1rem"><span class="badge ${u.role==='admin'?'b-gold':'b-sky'}">${u.role==='admin'?'👑 Admin':'✈️ Viajante'}</span></td>
      <td style="padding:.85rem 1rem;font-size:.78rem;color:var(--text-3)">${u.firstLogin?'⏳ Aguardando':'✅ Ativo'}</td>
      <td style="padding:.85rem 1rem;white-space:nowrap">
        <button class="btn btn-outline btn-sm" onclick="editGlobalUser('${u.cpf}')">✏️</button>
        ${u.cpf!=='10127544135'?`<button class="btn btn-danger btn-sm" onclick="deleteGlobalUser('${u.cpf}')">🗑️</button>`:''}
      </td>
    </tr>`).join('');
  ov.innerHTML=`
    <div class="modal" style="max-width:680px;max-height:92vh">
      <div class="modal-head">
        <div>
          <div class="modal-title">🗂️ Cadastro Global de Viajantes</div>
        ...
        <div style="display:flex;gap:.5rem;align-items:center">
          <button class="btn btn-red btn-sm" onclick="openNewGlobalUser()">➕ Novo</button>
          <button class="modal-x" onclick="document.getElementById('global-cad-overlay').remove();document.body.style.overflow='';openAdminTripSelector()">✕</button>
        </div>
      </div>
...
window.openNewGlobalUser=function(editCpf=null){
  ...
  ov.innerHTML=`
    <div class="modal" style="max-width:560px;max-height:92vh;overflow-y:auto">
      <div class="modal-head">
        <span class="modal-title">${u?'✏️ Editar Usuário':'➕ Novo Usuário'}</span>
        <button class="modal-x" onclick="document.getElementById('global-user-overlay').remove();document.body.style.overflow='';(document.getElementById('users-tbody')?renderCadastrosPage():renderGlobalCadastrosOverlay())">✕</button>
      </div>
      <div class="modal-body">
        ...
        <div class="g2" style="gap:.85rem">
          <div class="form-group dark-label" style="grid-column:1/-1">
            ...
            <div class="inp-wrap"><span class="inp-icon dark">👤</span>
          ...
          <div class="form-group dark-label" style="grid-column:1/-1">
            ...
            <select id="gu-role" class="form-control light no-icon">
              <option value="traveler" ${(!u||u.role==='traveler')?'selected':''}>✈️ Viajante</option>
        ...
        <div style="display:flex;align-items:center;gap:.75rem;padding:.75rem 0;border-top:1px solid var(--border);border-bottom:1px solid var(--border);margin:.75rem 0">
          ...
          <label for="gu-married" style="font-size:.88rem;font-weight:600;color:var(--text);cursor:pointer">💍 Casado(a)</label>
        </div>
        ...
        <div style="display:flex;align-items:center;gap:.75rem;padding:.75rem 0;border-bottom:1px solid var(--border);margin-bottom:.75rem">
          ...
          <label for="gu-kids" style="font-size:.88rem;font-weight:600;color:var(--text);cursor:pointer">👶 Tem filhos</label>
        </div>
      ...
      <div class="modal-foot">
        ...
        <button class="btn btn-red" onclick="saveGlobalUser()">💾 Salvar</button>
      </div>
    </div>`;
  document.body.appendChild(ov);
  // Wire events
...
function openCreateTrip(editId=null) {
  ...
  ov.innerHTML = `
    <div class="modal" style="max-width:640px;max-height:92vh;overflow-y:auto">
      <div class="modal-head">
        <span class="modal-title">${editId?'✏️ Editar Viagem':'➕ Nova Viagem'}</span>
        <button class="modal-x" onclick="closeCreateTrip()">✕</button>
      </div>
      <div class="modal-body">
        ...
        <div class="g2" style="gap:.85rem">
          <div class="form-group dark-label" style="grid-column:1/-1">
            ...
            <div class="inp-wrap"><span class="inp-icon dark">✈️</span>
          ...
          <div class="form-group dark-label" style="grid-column:1/-1">
            <label>🎯 Meta de Arrecadação (R$) <span style="font-size:.72rem;color:var(--text-3);font-weight:400">— deixe em branco para calcular automaticamente (preço × viajantes)</span></label>
            <div class="inp-wrap"><span class="inp-icon dark">🎯</span>
        ...
        <div style="margin:.75rem 0;padding:.75rem 1rem;background:var(--surface-2);border-radius:var(--r-sm);border:1px solid var(--border)">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.75rem">
            <span style="font-size:.85rem;font-weight:700;color:var(--text)">🚌 Ônibus</span>
      ...
      <div class="modal-foot">
        ...
        <button class="btn btn-red" onclick="saveTrip()">💾 Salvar Viagem</button>
      </div>
    </div>`;
  document.body.appendChild(ov);
...
function buildBusRow(b, i) {
  ...
  return `<div class="bus-row" data-bus="${b.id}" style="margin-bottom:.65rem;padding:.75rem;background:var(--surface);border:1px solid var(--border);border-radius:var(--r-sm)">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.6rem">
      <span style="font-size:.82rem;font-weight:700;color:var(--text)">🚌 Ônibus ${i+1}</span>
      <button type="button" class="btn btn-danger btn-sm btn-icon" onclick="removeBusRow(this)">🗑️</button>
    </div>
    ...
    document.querySelectorAll('.trip-card').forEach(card=>{
      ...
      btns.innerHTML=`
        <button class="btn btn-outline btn-sm btn-icon" title="Editar"
          onclick="event.stopPropagation();openCreateTrip('${id}')">✏️</button>
        <button class="btn btn-danger btn-sm btn-icon" title="Excluir"
          onclick="event.stopPropagation();deleteTrip('${id}')">🗑️</button>`;
...
function renderRecentTable(trip) {
  ...
  tbody.innerHTML=users.map(u=>{
    ...
    return`<tr>
      ...
      <td><button class="btn btn-outline btn-sm" onclick="viewTraveler('${u.cpf}')">👁️ Ver</button></td>
    </tr>`;
...
window.downloadTicketPDF = function() {
  ...
  <div class="ticket">
    <div class="tk-top">
      <div class="tk-logo">✝️ Igreja Viagens</div>
...
function initViajantes() {
  ...
  document.getElementById('nt-kids-qty')?.addEventListener('input',function(){
    ...
    for(let i=1;i<=Math.min(parseInt(this.value)||0,10);i++){
      c.innerHTML+=`<div class="form-group dark-label"><label>Nome do filho ${i}</label><div class="inp-wrap"><span class="inp-icon dark">👶</span><input type="text" id="kid-${i}" class="form-control light" placeholder="Nome do filho ${i}"></div></div>`;
    }
  });
...
function openTravelerModal(cpf) {
  ...
  document.getElementById('modal-trav-title').textContent = u?'✏️ Editar Viajante':'➕ Cadastrar Viajante';
...
window.openAddTravelerToTrip = function() {
    ...
    <div onclick="addUserToTrip('${u.cpf}')"
      ...
      <div style="flex:1">
        ...
        <div class="tm" style="font-size:.72rem">${CPF.format(u.cpf)}
          ${u.married&&u.spouseName?` · 💍 ${u.spouseName}`:''}
          ${u.hasKids&&u.kids?.length?` · 👶 ${u.kids.join(', ')}`:''}
        </div>
      </div>
  ...
  ov.innerHTML=`
    <div class="modal" style="max-width:500px">
      <div class="modal-head">
        <div>
          <div class="modal-title">➕ Adicionar à Viagem</div>
          <div style="font-size:.76rem;color:var(--text-3);margin-top:.18rem">Selecione viajantes já cadastrados no sistema</div>
        </div>
        <button class="modal-x" onclick="document.getElementById('add-to-trip-overlay').remove();document.body.style.overflow=''">✕</button>
      </div>
      <div class="modal-body" style="display:flex;flex-direction:column;gap:.5rem;max-height:55vh;overflow-y:auto">
        ${rows||`<div class="alert al-sky"><span class="al-ico">ℹ️</span><span>
          ...
          <button onclick="document.getElementById('add-to-trip-overlay').remove();document.body.style.overflow='';openGlobalCadastros()" class="btn btn-outline btn-sm" style="margin-top:.5rem">
            🗂️ Cadastrar Novo Viajante no Sistema
          </button>
        </span></div>`}
      </div>
      <div class="modal-foot">
        ...
        <button class="btn btn-outline btn-sm" onclick="document.getElementById('add-to-trip-overlay').remove();document.body.style.overflow='';openGlobalCadastros()">🗂️ Ir aos Cadastros</button>
      </div>
    </div>`;
  document.body.appendChild(ov);
};
...
function renderTravTable(filter='') {
  ...
  tbody.innerHTML=users.map(u=>{
    ...
    return`<tr>
      ...
      <td>${u.married?'💍 '+u.spouse:'—'}</td>
      <td>${u.hasKids?'👶 '+(u.kids||[]).length+' filho(s)':'—'}</td>
      <td><span class="badge ${pct===100?'b-green':pct>0?'b-gold':'b-red'}">${pct}%</span></td>
      <td>${pr>0?`<span class="badge b-gold">⏳ ${pr}</span>`:'<span class="badge b-muted">—</span>'}</td>
      <td><div class="flex g1">
        <button class="btn btn-outline btn-sm" onclick="openTravelerModal('${u.cpf}')">✏️</button>
        <button class="btn btn-outline btn-sm" onclick="viewTraveler('${u.cpf}')">👁️</button>
        <button class="btn btn-danger btn-sm" onclick="delTraveler('${u.cpf}')">🗑️</button>
      </div></td>
    </tr>`;
...
function buildReceiptHTML(receipts, cpf){
  ...
  return entries.map(([k,r])=>{
    ...
    if(hasData&&!isPdf){
      ...
      thumb=`<div style="width:64px;height:64px;border-radius:8px;background:rgba(232,25,44,.08);border:2px solid var(--border);display:flex;align-items:center;justify-content:center;font-size:1.6rem;flex-shrink:0;cursor:pointer" onclick="openReceiptViewer('${cpf}','${k}')">📄</div>`;
    } else {
      thumb=`<div style="width:64px;height:64px;border-radius:8px;background:var(--gold-light);border:2px dashed var(--gold);display:flex;align-items:center;justify-content:center;font-size:1.4rem;flex-shrink:0">🖼️</div>`;
    }
    return `
    <div style="display:flex;align-items:center;gap:.85rem;padding:.85rem;border:1.5px solid var(--border);border-radius:var(--r-sm);background:var(--surface);margin-bottom:.5rem;flex-wrap:wrap">
      ...
      <div style="flex:1;min-width:120px">
        ...
        ${!hasData?`<p style="font-size:.72rem;color:#a36c00;font-weight:600;margin-top:.2rem">⚠️ Peça reenvio</p>`:''}
      </div>
      <div style="display:flex;flex-direction:column;gap:.35rem;align-items:flex-end;flex-shrink:0">
        <span class="badge ${r.status==='approved'?'b-green':r.status==='rejected'?'b-red':'b-gold'}">${r.status==='approved'?'✅ Aprovado':r.status==='rejected'?'❌ Recusado':'⏳ Pendente'}</span>
        ${r.status==='rejected'&&r.note?`<p style="font-size:.72rem;color:var(--coral);text-align:right;max-width:180px">⚠️ ${r.note}</p>`:''}
        <div style="display:flex;gap:.35rem">
          ${hasData?`<button class="btn btn-navy btn-sm" onclick="openReceiptViewer('${cpf}','${k}')">🔍 Ver</button>`:''}
          ${r.status!=='approved'?`<button class="btn btn-success btn-sm" onclick="approveR('${cpf}','${k}')">✔ Aprovar</button>`:''}
          ${r.status!=='approved'?`<button class="btn btn-danger btn-sm" onclick="promptRejectReceipt('${cpf}','${k}')" title="Recusar com observação">❌ Recusar</button>`:''}
        </div>
      </div>
    </div>`;
  }).join('');
...
function openReceiptViewer(cpf,parcel){
  ...
  viewer.innerHTML=`
    <div style="display:flex;align-items:center;justify-content:space-between;width:100%;max-width:900px;margin-bottom:1rem">
      <div style="color:#fff;font-size:.9rem;font-weight:600">🧾 Parcela ${parcel} &nbsp;|&nbsp; <span style="color:rgba(255,255,255,.5)">${r.filename||''}</span></div>
      <div style="display:flex;gap:.6rem">
        ...
        <button onclick="document.getElementById('receipt-viewer').remove()" style="padding:.45rem .9rem;background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.2);color:#fff;border-radius:6px;font-size:.8rem;cursor:pointer">✕ Fechar</button>
      </div>
    </div>
    ${isPdf
...
function renderTravPay(s, tripId){
  ...
  if(!trip.price){
    ...
    if(el) el.innerHTML=`<div class="alert al-gold"><span class="al-ico">⏳</span><span>O administrador ainda não configurou o valor da viagem.</span></div>`;
    return;
  }
  ...
  el.innerHTML=Array.from({length:p.totalInstallments},(_,i)=>{
    ...
    return`<div class="parcel">
      ...
      <div class="parcel-inf">
        ...
        ${r?.status==='rejected'&&r?.note?`<p style="font-size:.76rem;color:var(--coral);margin-top:.2rem">⚠️ Admin: "${r.note}" — <strong>Reenvie o comprovante</strong></p>`:''}
      </div>
      <span class="parcel-val">${fmt(each)}</span>
      <span class="badge ${st==='approved'?'b-green':st==='pending'?'b-gold':'b-muted'}">${st==='approved'?'✅ Aprovado':st==='pending'?'⏳ Aguardando':'📤 Enviar'}</span>
      ${st==='rejected'?`<button class="btn btn-red btn-sm" onclick="openUpload(${n})">🔄 Reenviar</button>`:st!=='approved'?`<button class="btn btn-outline btn-sm" onclick="openUpload(${n})">📎</button>`:''}
    </div>`;
  }).join('');
...
window.submitReceipt = function(){
  ...
  reader.onload=e=>{
    ...
    if(btn){ btn.disabled=false; btn.textContent='📤 Enviar Comprovante'; }
    closeM('upload-modal');
    const fi=document.getElementById('rcpt-file'); if(fi) fi.value='';
    toast('Comprovante enviado! Aguardando aprovação.','ok');
    renderParcels(s.cpf,tripId);
  };
  reader.onerror=()=>{ if(btn){btn.disabled=false;btn.textContent='📤 Enviar Comprovante';} toast('Erro ao ler arquivo.','er'); };
...
function renderAdminPay(tripId){
  ...
  tbody.innerHTML=users.map(u=>{
    ...
    return`<tr>
      ...
      <td>${pr>0?`<span class="badge b-gold">⏳ ${pr}</span>`:'<span class="badge b-green">Em dia</span>'}</td>
...
function renderBusAdmin(trip, tripId){
  ...
  if(!trip||!(trip.buses||[]).length){
    container.innerHTML=`<div class="alert al-gold"><span class="al-ico">⚠️</span><span>Nenhum ônibus configurado nesta viagem.</span></div>`;
    return;
  }
  const seats=DB.get('iv_seats')||{};
  const users=(DB.get('iv_users')||[]).filter(u=>(trip.travelers||[]).includes(u.cpf));
  ...
  container.innerHTML=trip.buses.map(bus=>{
    ...
    for(let fl=1;fl<=floors;fl++){
        ...
        for(let c=0;c<4;c++){
          ...
          if(sn>flSeats){ seatsHtml+='<div style="width:36px"></div>'; }
            ...
            seatsHtml+=`<button class="seat ${cls}" title="${ttl}"
              ${isDrv?'disabled':''}
              onclick="adminClickSeat('${tripId}','${bus.id}',${n},${fl})">${isDrv?'🚗':n}</button>`;
            sn++;
          }
        }
        seatsHtml+='</div>';
      ...
      busHtml+=`<div style="margin-bottom:1.25rem">
        <div style="font-size:.78rem;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:var(--text-3);margin-bottom:.65rem">
          ${floors===2?(fl===1?'🚌 1º Andar':'🏢 2º Andar'):'🚌 Mapa de Assentos'}
        </div>
    ...
    return`<div class="card mb3">
      <div class="card-head">
        <div><div class="card-title">🚌 ${floors===2?'Ônibus 2 andares':'Ônibus'} #${bus.id} — ${total} assentos</div>
...
function showTravelerPicker(tripId, busId, seatNum, floor=1, trip){
  ...
  ov.innerHTML=`
    <div class="modal" style="max-width:420px">
      <div class="modal-head"><span class="modal-title">🪑 Atribuir Assento ${seatNum}</span>
        <button class="modal-x" onclick="document.getElementById('traveler-picker').remove();document.body.style.overflow=''">✕</button>
      </div>
...
function renderHotelAdmin(trip, tripId){
  ...
  if(!hotels.length){
    container.innerHTML=`<div class="alert al-sky"><span class="al-ico">ℹ️</span><span>Nenhum hotel configurado. Adicione um hotel usando o formulário abaixo.</span></div>`;
    return;
  }
  ...
  container.innerHTML=hotels.map(hotel=>`
    <div class="card mb3">
      <div class="card-head">
        <div><div class="card-title">🏨 ${hotel.name}</div>
          <div class="card-sub">${(hotel.rooms||[]).length} quartos</div></div>
        <button class="btn btn-outline btn-sm" onclick="openAddRoom('${tripId}','${hotel.id}')">➕ Quarto</button>
      </div>
      <div class="card-body">
          ...
          ${(hotel.rooms||[]).map(room=>{
            ...
            return`<div class="room ${occs.length>=room.capacity?'occ':''}">
              ...
              <div class="mt1" style="display:flex;gap:.35rem;flex-wrap:wrap">
                <button class="btn btn-outline btn-sm" style="font-size:.72rem" onclick="openAssignRoom('${tripId}','${hotel.id}','${room.id}')">👤 Atribuir</button>
                <button class="btn btn-danger btn-sm" style="font-size:.72rem" onclick="removeRoom('${tripId}','${hotel.id}','${room.id}')">🗑️</button>
              </div>
            </div>`;
...
window.openAddRoom = function(tripId, hotelId){
  ...
  ov.innerHTML=`
    <div class="modal" style="max-width:380px">
      <div class="modal-head"><span class="modal-title">➕ Novo Quarto</span>
        <button class="modal-x" onclick="document.getElementById('add-room-overlay').remove();document.body.style.overflow=''">✕</button>
      </div>
      <div class="modal-body">
        <div class="form-group dark-label">
        ...
        <div class="form-group dark-label">
          ...
          <select id="room-type" class="form-control light">
            <option value="single">🛏️ Individual (1 pessoa)</option>
...
window.openAssignRoom = function(tripId, hotelId, roomId){
  ...
  ov.innerHTML=`
    <div class="modal" style="max-width:440px">
      <div class="modal-head">
        <div><div class="modal-title">🏨 Quarto ${roomId}</div>
        ...
        <button class="modal-x" onclick="document.getElementById('assign-room-overlay').remove();document.body.style.overflow=''">✕</button>
      </div>
      <div class="modal-body">
          ...
          ${users.map(u=>{
            ...
            return`<label style="display:flex;align-items:center;gap:.75rem;padding:.65rem 1rem;
              ...
              <div style="flex:1">
                ...
                ${u.married&&u.spouseName?`<div class="tm" style="font-size:.72rem">💍 ${u.spouseName}${u.hasKids&&u.kids?.length?' · 👶 '+u.kids.join(', '):''}</div>`:''}
      ...
      <div class="modal-foot">
        ...
        <button class="btn btn-red" onclick="saveRoomAssignment('${tripId}','${roomId}',${room.capacity})">💾 Salvar</button>
      </div>
    </div>`;
  document.body.appendChild(ov);
};
...
function renderCadastrosPage(filter='') {
  ...
  tbody.innerHTML=users.map(u=>`
    <tr>
      ...
      <td><span class="badge ${u.role==='admin'?'b-gold':'b-sky'}">${u.role==='admin'?'👑 Admin':'✈️ Viajante'}</span></td>
      <td><span class="badge ${u.firstLogin?'b-gold':'b-green'}">${u.firstLogin?'⏳ Aguardando':'✅ Ativo'}</span></td>
      <td style="font-size:.8rem;color:var(--text-3)">${getUserTrips(u.cpf).map(t=>t.name).join(', ')||'—'}</td>
      <td style="white-space:nowrap">
        <button class="btn btn-outline btn-sm" onclick="editGlobalUser('${u.cpf}')">✏️ Editar</button>
        ${u.cpf!=='10127544135'?`<button class="btn btn-danger btn-sm" onclick="deleteGlobalUser('${u.cpf}')">🗑️</button>`:''}
      </td>
...
window.debugIV = function(){
  ...
  users.forEach(u=>console.log(`${u.role==='admin'?'👑':'✈️'} ${u.name} | ${CPF.format(u.cpf)} | ${u.password} | firstLogin:${u.firstLogin}`));
